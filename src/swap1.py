...
try:
    ...
    class InsufficientBalance(Exception):
        def __init__(self, had: int, needed: int) -> None:
            Exception.__init__(
                self, f"Insufficient balance. Had {had}, needed {needed}"
            )
    ...
    class Uniswap:
        def __init__(
            ...
            if self.version == 1:
                ...
                self.factory_contract = self._load_contract(
                    ...
                    address=factory_contract_address_v2,
                )
        ...
        def add_liquidity(
            ...
            function = self.exchange_contract(token).functions.addLiquidity(
                *func_params
            )
        ...
        def remove_liquidity(self, gwei, token: str, max_token: int) -> HexBytes:
            ...
            function = self.exchange_contract(token).functions.removeLiquidity(
                *func_params
            )
            return self._build_and_send_tx(
                gwei, my_address, my_pk, my_address, my_pk, function
            )
        ...
        def make_trade(
            ...
            if input_token == ETH_ADDRESS:
                return self._eth_to_token_swap_input(
                    gwei, my_address, my_pk, output_token, Wei(qty), recipient
                )
            else:
        ...
        def _eth_to_token_swap_input(
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(
                    gwei, my_address, my_pk, function, tx_params
                )
            else:
        ...
        # ------ Approval Utils ------------------------------------------------------------
        def _get_tx_params2(
            self, value: Wei = Wei(0), gas: Wei = Wei(250000)
        ) -> TxParams:
        ...
        def _get_tx_params(
            self, gwei, my_address, value: Wei = Wei(0), gas: Wei = Wei(250000)
        ) -> TxParams:
            """Get generic transaction parameters."""
            return {
                "from": my_address,
        ...
        def _buy_test_assets(self) -> None:
            ...
            for token_name in ["BAT", "DAI"]:
                ...
                price = self.get_eth_token_output_price(
                    _str_to_addr(token_addr), TEST_AMT
                )
